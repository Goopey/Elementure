	...
	public static void execute(LevelAccessor world, double x, double y, double z) {
		if (Math.random() < 0.5) {
			...
			if (world instanceof ServerLevel _serverworld) {
				...
				if (template != null) {
					template.placeInWorld(_serverworld, new BlockPos(x - 4, y - 2, z - 4), new BlockPos(x - 4, y - 2, z - 4), new StructurePlaceSettings().setRotation(Rotation.NONE).setMirror(Mirror.NONE).setIgnoreEntities(false),
							_serverworld.random, 3);
				}
			}
		}
		...
		if (Math.random() < 0.4) {
			if (Math.random() < 0.2) {
				world.setBlock(new BlockPos(x + Math.random() * 6 - 3, y + 1, z + Math.random() * 6 - 3), ElementureModBlocks.STARFLOWER.get().defaultBlockState(), 3);
			} else {
				world.setBlock(new BlockPos(x + Math.random() * 6 - 3, y + 1, z + Math.random() * 6 - 3), ElementureModBlocks.SOLARLILY.get().defaultBlockState(), 3);
			}
		}
