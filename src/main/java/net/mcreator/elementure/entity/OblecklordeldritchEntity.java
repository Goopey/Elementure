...
public class OblecklordeldritchEntity extends Monster implements IAnimatable {
	...
	public OblecklordeldritchEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ElementureModEntities.OBLECKLORDELDRITCH.get(), world);
	}

	public OblecklordeldritchEntity(EntityType<OblecklordeldritchEntity> type, Level world) {
		super(type, world);
		xpReward = 81;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(2, new Goal() {
			...
			public boolean canUse() {
				if (OblecklordeldritchEntity.this.getTarget() != null && !OblecklordeldritchEntity.this.getMoveControl().hasWanted()) {
					return true;
				} else {
					return false;
				}
			...
			public boolean canContinueToUse() {
				return OblecklordeldritchEntity.this.getMoveControl().hasWanted() && OblecklordeldritchEntity.this.getTarget() != null
						&& OblecklordeldritchEntity.this.getTarget().isAlive();
			}

			@Override
			public void start() {
				LivingEntity livingentity = OblecklordeldritchEntity.this.getTarget();
				Vec3 vec3d = livingentity.getEyePosition(1);
				OblecklordeldritchEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 1);
			}

			@Override
			public void tick() {
				LivingEntity livingentity = OblecklordeldritchEntity.this.getTarget();
				if (OblecklordeldritchEntity.this.getBoundingBox().intersects(livingentity.getBoundingBox())) {
					OblecklordeldritchEntity.this.doHurtTarget(livingentity);
				} else {
					double d0 = OblecklordeldritchEntity.this.distanceToSqr(livingentity);
					if (d0 < 64) {
						Vec3 vec3d = livingentity.getEyePosition(1);
						OblecklordeldritchEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 1);
					}
	...
	public void playerTouch(Player sourceentity) {
		...
		OblecklordeldritchRevertProcedure.execute(this.level, this.getX(), this.getY(), this.getZ(), this);
	}
