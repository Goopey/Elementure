...
import net.mcreator.elementure.procedures.SpikercrabDisplayProcedure;
...
public class SpikercrabOverlayOverlay {
	...
	public static void eventHandler(RenderGameOverlayEvent.Pre event) {
		if (event.getType() == RenderGameOverlayEvent.ElementType.ALL) {
			...
			if (SpikercrabDisplayProcedure.execute(entity)) {
				...
				Minecraft.getInstance().gui.blit(event.getMatrixStack(), posX + -57, posY + 68 - (int) FishPosReturnProcedure.execute(entity), 0, 0,
						16, 16, 16, 16);
				if (FishabilitytimerReturnProcedure.execute(entity) < 60) {
					RenderSystem.setShaderTexture(0, new ResourceLocation("elementure:textures/screens/spiker_crab_pincers.png"));
					Minecraft.getInstance().gui.blit(event.getMatrixStack(), posX + -71,
							posY + 30 - (int) FishabilityposReturnProcedure.execute(entity), 0, 0, 44, 44, 44, 44);
				}
			}
