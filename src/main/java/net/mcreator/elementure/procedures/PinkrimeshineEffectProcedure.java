		...
		if (itemstack.getItem() == ElementureModItems.RAREPINKRIMESHINE.get() || itemstack.getItem() == ElementureModItems.PINKRIMESHINE.get()) {
			{
				...
				for (Entity entityiterator : _entfound) {
					if (entityiterator instanceof LivingEntity _entity)
						_entity.addEffect(
								new MobEffectInstance(MobEffects.CONFUSION, (int) (itemstack.getOrCreateTag().getDouble("duration") * 2 * (1 + itemstack.getOrCreateTag().getDouble("potency"))), (int) itemstack.getOrCreateTag().getDouble("potency")));
				}
			}
			if (world instanceof ServerLevel _level)
				_level.sendParticles(ParticleTypes.CAMPFIRE_SIGNAL_SMOKE, (entity.getX()), (entity.getY()), (entity.getZ()), (int) (itemstack.getOrCreateTag().getDouble("duration") * 7), (2 + itemstack.getOrCreateTag().getDouble("potency")),
