...
import net.mcreator.elementure.procedures.DarkchoirpopupConfirmProcedure;

import com.mojang.blaze3d.systems.RenderSystem;
import com.mojang.blaze3d.platform.GlStateManager;

@Mod.EventBusSubscriber({Dist.CLIENT})
public class DarkchoirPopupOverlay {
	...
	public static void eventHandler(RenderGameOverlayEvent.Pre event) {
		if (event.getType() == RenderGameOverlayEvent.ElementType.ALL) {
			...
			if (DarkchoirpopupConfirmProcedure.execute(entity)) {
				...
				Minecraft.getInstance().font.draw(event.getMatrixStack(), "Dark Choir", posX + -198, posY + -116, -203);
				Minecraft.getInstance().font.draw(event.getMatrixStack(), "By:Mattia Cupelli", posX + -198, posY + -108, -203);
			}
