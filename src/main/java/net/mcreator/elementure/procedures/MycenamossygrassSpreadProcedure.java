...
public class MycenamossygrassSpreadProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		double CoordsX = 0;
		double CoordsY = 0;
		double CoordsZ = 0;
		if (!((world.getBlockState(new BlockPos(x, y + 1, z))).getBlock() == ElementureModBlocks.MYCENASHROOMBLOCK.get() || (world.getBlockState(new BlockPos(x, y + 1, z))).getBlock() == Blocks.AIR
				|| (world.getBlockState(new BlockPos(x, y + 1, z))).getBlock() == ElementureModBlocks.MYCENASPROUTS.get() || (world.getBlockState(new BlockPos(x, y + 1, z))).getBlock() == ElementureModBlocks.MYCENAVINE.get())
				&& Math.random() < 0.024) {
		...
		for (int index0 = 0; index0 < (int) (729); index0++) {
			if (16 * CoordsY * CoordsY + CoordsX * CoordsX + CoordsZ * CoordsZ < 16 && (world.getBlockState(new BlockPos(x + CoordsX, y + CoordsY, z + CoordsZ))).getBlock() == Blocks.DIRT
				...
				world.setBlock(new BlockPos(x + CoordsX, y + CoordsY, z + CoordsZ), ElementureModBlocks.MYCENAMOSSYGRASS.get().defaultBlockState(), 3);
			}
			CoordsX = CoordsX + 1;
			if (CoordsX == 4) {
				CoordsX = -4;
