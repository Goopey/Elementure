...
public class GreedierbagItem extends Item {
	...
	public void appendHoverText(ItemStack itemstack, Level world, List<Component> list, TooltipFlag flag) {
		...
		list.add(new TextComponent("\"Greed is what I am"));
		list.add(new TextComponent("'Cause no one can"));
		list.add(new TextComponent("Hide from their sins"));
		list.add(new TextComponent("There's no escape"));
		list.add(new TextComponent("Even if you try to run away you still must pay\""));
		list.add(new TextComponent("-Greed Jan 31"));
		list.add(new TextComponent("2021"));
	}
		...
		if (entity instanceof ServerPlayer serverPlayer) {
			NetworkHooks.openGui(serverPlayer, new MenuProvider() {
				...
				public Component getDisplayName() {
					return new TextComponent("Greedier's bag");
				}
