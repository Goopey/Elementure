...
import net.mcreator.elementure.procedures.GlassbellboatActivateInventoryProcedure;
...
public class GlassbellboatEntityEntity extends PathfinderMob {
	public GlassbellboatEntityEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ElementureModEntities.GLASSBELLBOAT_ENTITY.get(), world);
	}

	public GlassbellboatEntityEntity(EntityType<GlassbellboatEntityEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
		...
		this.moveControl = new MoveControl(this) {
			...
			public void tick() {
				if (GlassbellboatEntityEntity.this.isInWater())
					GlassbellboatEntityEntity.this.setDeltaMovement(GlassbellboatEntityEntity.this.getDeltaMovement().add(0, 0.005, 0));
				if (this.operation == MoveControl.Operation.MOVE_TO && !GlassbellboatEntityEntity.this.getNavigation().isDone()) {
					double dx = this.wantedX - GlassbellboatEntityEntity.this.getX();
					...
					if (GlassbellboatEntityEntity.this.isInWater()) {
						...
						float f3 = Mth.cos(GlassbellboatEntityEntity.this.getXRot() * (float) (Math.PI / 180.0));
						GlassbellboatEntityEntity.this.setZza(f3 * f1);
						GlassbellboatEntityEntity.this.setYya((float) (f1 * dy));
					} else {
						GlassbellboatEntityEntity.this.setSpeed(f1 * 0.05F);
					}
				} else {
					GlassbellboatEntityEntity.this.setSpeed(0);
					GlassbellboatEntityEntity.this.setYya(0);
					GlassbellboatEntityEntity.this.setZza(0);
				}
			}
		};
	}
			...
			if (sourceentity instanceof ServerPlayer serverPlayer) {
				NetworkHooks.openGui(serverPlayer, new MenuProvider() {
					...
					public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
						...
						packetBuffer.writeVarInt(GlassbellboatEntityEntity.this.getId());
						return new GlassbellboatGUIMenu(id, inventory, packetBuffer);
					}
				}, buf -> {
	...
	public void baseTick() {
		super.baseTick();
		GlassbellboatActivateInventoryProcedure.execute(this.level, this);
	}
