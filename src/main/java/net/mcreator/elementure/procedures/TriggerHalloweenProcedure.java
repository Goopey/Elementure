...
public class TriggerHalloweenProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world) {
		double day = 0;
		double month = 0;
		if (Calendar.getInstance().get(Calendar.MONTH) == 9 && Calendar.getInstance().get(Calendar.DAY_OF_MONTH) >= 24
				|| Calendar.getInstance().get(Calendar.MONTH) == 10 && Calendar.getInstance().get(Calendar.DAY_OF_MONTH) <= 1) {
