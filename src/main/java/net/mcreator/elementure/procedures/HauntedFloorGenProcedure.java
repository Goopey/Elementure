...
public class HauntedFloorGenProcedure {
	/*
	 ...
	 * after a big one
	 */
	final static int MAX_FLOOR_SIZE = ((int) Math.random() * 12 + 40);
	...
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		for (int i = FLOOR_START; i < FLOOR_SIZE; i++) {
			if (i != FLOOR_MID && endRooms[i]) {
				 ...
				 */
				if (endRoomCount == 0) {
					SwordtempleEndRoomSecret1Procedure.execute(world, x + (9 * xPos), y, z + (9 * zPos));
	...
	public static boolean canPlaceRoom(int neighbors, int currentRoom, boolean[] floorArray, boolean[] queueArray) {
		...
		if (Math.random() < 0.5 && currentRoom != 45) {
			return false;
		}
		return true;
	}
