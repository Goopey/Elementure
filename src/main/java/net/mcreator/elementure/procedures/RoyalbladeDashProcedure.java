...
public class RoyalbladeDashProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if ((entity.getCapability(ElementureModVariables.PLAYER_VARIABLES_CAPABILITY, null)
				.orElse(new ElementureModVariables.PlayerVariables())).light_essence >= 22) {
			{
			...
			for (int index0 = 0; index0 < (int) (32); index0++) {
				world.addParticle(ParticleTypes.SWEEP_ATTACK, (entity.getX() + 0.85 * yaw * pitch + (Math.random() - 0.5) * 3),
						(entity.getY() + 1.633 + (Math.random() - 0.5) / 2), ((Math.random() - 0.5) * 3 + entity.getZ() + 0.85 * yaw2 * pitch), 0, 0,
						0);
			}
			{
			...
			for (int index1 = 0; index1 < (int) (16); index1++) {
				world.addParticle(ParticleTypes.SWEEP_ATTACK, (entity.getX() + 4.2 * yaw * pitch + (Math.random() - 0.5) * 3),
						(entity.getY() + 1.633 + (Math.random() - 0.5) / 2), ((Math.random() - 0.5) * 3 + entity.getZ() + 4.2 * yaw2 * pitch), 0, 0,
						0);
			}
			{
			...
			for (int index2 = 0; index2 < (int) (16); index2++) {
				world.addParticle(ParticleTypes.SWEEP_ATTACK, (entity.getX() + 10.2 * yaw * pitch + (Math.random() - 0.5) * 3),
						(entity.getY() + 1.633 + (Math.random() - 0.5) / 2), ((Math.random() - 0.5) * 3 + entity.getZ() + 10.2 * yaw2 * pitch), 0, 0,
						0);
			}
			entity.setDeltaMovement(new Vec3((entity.getDeltaMovement().x() + 5 * yaw * pitch), (entity.getDeltaMovement().y()),
					(entity.getDeltaMovement().z() + 5 * yaw2 * pitch)));
		}
	}
}
