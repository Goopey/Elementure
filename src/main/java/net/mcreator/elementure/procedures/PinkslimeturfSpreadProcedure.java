...
public class PinkslimeturfSpreadProcedure {
		...
		if (Math.random() < 0.00164) {
			...
			for (int index0 = 0; index0 < (int) (125); index0++) {
				if (1.21 * CoordsY * CoordsY + CoordsX * CoordsX + CoordsZ * CoordsZ < 4
					...
					world.setBlock(new BlockPos(x + CoordsX, y + CoordsY, z + CoordsZ), ElementureModBlocks.PINKSLIMETURF.get().defaultBlockState(),
							3);
				}
				CoordsX = CoordsX + 1;
				if (CoordsX == 2) {
