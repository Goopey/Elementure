	...
	public static void execute(LevelAccessor world, double x, double y, double z) {
		if (Math.random() < 0.6) {
			if (world instanceof ServerLevel _serverworld) {
				...
				if (template != null) {
					template.placeInWorld(_serverworld, new BlockPos(x + 1, y, z + 1), new BlockPos(x + 1, y, z + 1), new StructurePlaceSettings().setRotation(Rotation.CLOCKWISE_180).setMirror(Mirror.NONE).setIgnoreEntities(false),
