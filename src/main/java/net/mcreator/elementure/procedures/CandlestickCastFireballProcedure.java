	...
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (CandlestickGlowProcedure.execute(entity)) {
			if (entity.isShiftKeyDown()) {
				essenceTreshhold = Math.round((entity.getCapability(ElementureModVariables.PLAYER_VARIABLES_CAPABILITY, null)
				...
				if (world instanceof ServerLevel _level)
					_level.sendParticles(ParticleTypes.FLAME,
							(entity.level.clip(new ClipContext(entity.getEyePosition(1f),
									entity.getEyePosition(1f).add(entity.getViewVector(1f).scale(essenceTreshhold)), ClipContext.Block.OUTLINE,
							...
							(int) Math.round(essenceTreshhold * 250 * essenceTreshhold), (essenceTreshhold / 4), (essenceTreshhold / 4),
							(essenceTreshhold / 4), 0.02);
				{
					...
					List<Entity> _entfound = world
							.getEntitiesOfClass(Entity.class, new AABB(_center, _center).inflate((essenceTreshhold * 2) / 2d), e -> true).stream()
				...
				if (world instanceof ServerLevel _level)
					_level.sendParticles(ParticleTypes.FLAME,
							(entity.level
									.clip(new ClipContext(entity.getEyePosition(1f), entity.getEyePosition(1f).add(entity.getViewVector(1f).scale(4)),
				...
				}
				{
					double _setval = (entity.getCapability(ElementureModVariables.PLAYER_VARIABLES_CAPABILITY, null)
							.orElse(new ElementureModVariables.PlayerVariables())).fire_essence - 45;
					entity.getCapability(ElementureModVariables.PLAYER_VARIABLES_CAPABILITY, null).ifPresent(capability -> {
						capability.fire_essence = _setval;
						capability.syncPlayerVariables(entity);
					});
