...
public class GlasslightbrewerPotions4Procedure {
		...
		if (properVer >= 4 && white + black >= 1) {
			if (white >= black) {
				if ((new Object() {
					...
					retItem = new ItemStack(ElementureModItems.RAREWHITERIMESHINE.get());
				} else {
					retItem = new ItemStack(ElementureModItems.WHITERIMESHINE.get());
				}
			} else {
				if ((new Object() {
					public ItemStack getItemStack(LevelAccessor world, BlockPos pos, int slotid) {
					...
					retItem = new ItemStack(ElementureModItems.RAREBLACKRIMESHINE.get());
				} else {
					retItem = new ItemStack(ElementureModItems.BLACKRIMESHINE.get());
				}
			}
			retItem.getOrCreateTag().putDouble("potency", gray);
			retItem.getOrCreateTag().putDouble("duration", ((1 + lightgray) * 600));
		}
		return retItem;
	}
}
