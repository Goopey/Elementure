...
public class ScribingStationGUIMenu extends AbstractContainerMenu implements Supplier<Map<Integer, Slot>> {
	...
	public ScribingStationGUIMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
		super(ElementureModMenus.SCRIBING_STATION_GUI.get(), id);
		...
		for (int si = 0; si < 3; ++si)
			for (int sj = 0; sj < 9; ++sj)
				this.addSlot(new Slot(inv, sj + (si + 1) * 9, 0 + 8 + sj * 18, -48 + 84 + si * 18));
		for (int si = 0; si < 9; ++si)
			this.addSlot(new Slot(inv, si, 0 + 8 + si * 18, -48 + 142));
	}
