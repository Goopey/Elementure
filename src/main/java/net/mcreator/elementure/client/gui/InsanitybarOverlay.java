	...
	public static void eventHandler(RenderGameOverlayEvent.Pre event) {
		if (event.getType() == RenderGameOverlayEvent.ElementType.ALL) {
			...
			if (InsanityIsInDimensionProcedure.execute(entity)) {
				...
				Minecraft.getInstance().gui.blit(event.getMatrixStack(), posX + -72, posY + -115, 0, 0, 144, 18, 144, 18);
				...
				if (InsanityNug7ConditionProcedure.execute(entity)) {
					...
					Minecraft.getInstance().gui.blit(event.getMatrixStack(), posX + -71, posY + -114, 0, 0, 21, 16, 21, 16);
				}
				if (InsanityNug6ConditionProcedure.execute(entity)) {
					...
					Minecraft.getInstance().gui.blit(event.getMatrixStack(), posX + -50, posY + -114, 0, 0, 20, 16, 20, 16);
				}
				if (InsanityNug5ConditionProcedure.execute(entity)) {
					RenderSystem.setShaderTexture(0, new ResourceLocation("elementure:textures/screens/insanity_nug_2.png"));
					Minecraft.getInstance().gui.blit(event.getMatrixStack(), posX + -30, posY + -114, 0, 0, 20, 16, 20, 16);
				}
				if (InsanityNug4ConditionProcedure.execute(entity)) {
					RenderSystem.setShaderTexture(0, new ResourceLocation("elementure:textures/screens/insanity_nug_2.png"));
					Minecraft.getInstance().gui.blit(event.getMatrixStack(), posX + -10, posY + -114, 0, 0, 20, 16, 20, 16);
				}
				...
				if (InsanityNug1ConditionProcedure.execute(entity)) {
					...
					Minecraft.getInstance().gui.blit(event.getMatrixStack(), posX + 50, posY + -114, 0, 0, 21, 16, 21, 16);
				}
			}
