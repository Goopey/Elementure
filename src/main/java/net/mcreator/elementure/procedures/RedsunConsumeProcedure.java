...
public class RedsunConsumeProcedure {
	public static void execute(LevelAccessor world, Entity entity, ItemStack itemstack) {
		...
		double pitch2 = 0;
		double yaw = 0;
		if (itemstack.getOrCreateTag().getBoolean("redSunActive") && (entity.getCapability(ElementureModVariables.PLAYER_VARIABLES_CAPABILITY, null)
				.orElse(new ElementureModVariables.PlayerVariables())).fire_essence > 0 && Math.random() < 0.025) {
			{
		...
		if ((entity.getCapability(ElementureModVariables.PLAYER_VARIABLES_CAPABILITY, null)
				.orElse(new ElementureModVariables.PlayerVariables())).fire_essence >= 120) {
			itemstack.getOrCreateTag().putBoolean("redSunActive", (false));
		}
		if ((entity.getCapability(ElementureModVariables.PLAYER_VARIABLES_CAPABILITY, null)
				.orElse(new ElementureModVariables.PlayerVariables())).fire_essence >= 60
		...
		if (itemstack.getOrCreateTag().getBoolean("redSunActive") || itemstack.getOrCreateTag().getDouble("redSunOverload") > 0) {
			...
			for (int index0 = 0; index0 < (int) (16); index0++) {
				world.addParticle(ParticleTypes.FLAME, (entity.getX() + (-0.2) * yaw * pitch), (entity.getY() + 1.633 + 0.2 * pitch2),
						(entity.getZ() + (-0.2) * yaw2 * pitch), (((yaw * pitch) / (-4) + (Math.random() - 0.5) / 2) / 16),
