...
public class MycenashroomlordSuperAttackBehavior1Procedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		double locZ = 0;
		double locX = 0;
		if (entity.getPersistentData().getDouble("attackongoingtime") == 60 || entity.getPersistentData().getDouble("attackongoingtime") == 120
				|| entity.getPersistentData().getDouble("attackongoingtime") == 180) {
			for (int index0 = 0; index0 < (int) (5); index0++) {
				...
				if (world instanceof ServerLevel _level) {
					...
					entityToSpawn.moveTo((entity.getX() + locX), (entity.getY() + 4), (entity.getZ() + locZ), world.getRandom().nextFloat() * 360F,
							0);
