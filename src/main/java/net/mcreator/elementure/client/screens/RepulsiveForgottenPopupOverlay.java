...
public class RepulsiveForgottenPopupOverlay {
	...
	public static void eventHandler(RenderGuiEvent.Pre event) {
		...
		if (RepulsiveForgottenpopupConditionProcedure.execute(entity)) {
			...
			Minecraft.getInstance().font.draw(event.getPoseStack(), Component.translatable("gui.elementure.repulsive_forgotten_popup.label_forgotten"), posX + -198, posY + -116, -203);
			Minecraft.getInstance().font.draw(event.getPoseStack(), Component.translatable("gui.elementure.repulsive_forgotten_popup.label_byrepulsive"), posX + -198, posY + -108, -203);
		}
