		...
		if (((entity.getCapability(ElementureModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ElementureModVariables.PlayerVariables())).relic_inventory_artifact_1).getItem() == relic_item.getItem()
				|| ((entity.getCapability(ElementureModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ElementureModVariables.PlayerVariables())).relic_inventory_artifact_2).getItem() == relic_item.getItem()) {
			{
				...
				for (Entity entityiterator : _entfound) {
					if ((entityiterator instanceof LivingEntity _livEnt ? _livEnt.hasEffect(MobEffects.POISON) : false) && (entityiterator instanceof LivingEntity _livEnt ? _livEnt.getHealth() : -1) > 1
						...
						poisonDamage = ((entityiterator instanceof LivingEntity _livEnt && _livEnt.hasEffect(MobEffects.POISON) ? _livEnt.getEffect(MobEffects.POISON).getAmplifier() : 0) + 1) * 0.8
								* (entityiterator instanceof LivingEntity _livEnt && _livEnt.hasEffect(MobEffects.POISON) ? _livEnt.getEffect(MobEffects.POISON).getDuration() : 0) * 0.05;
						if (entityiterator instanceof LivingEntity _entity)
							_entity.setHealth((float) Math.max(1, (entityiterator instanceof LivingEntity _livEnt ? _livEnt.getHealth() : -1) - Math.round(poisonDamage * 0.25)));
						if (entityiterator instanceof LivingEntity _entity)
