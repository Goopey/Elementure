			...
			for (int index0 = 0; index0 < (int) (125); index0++) {
				if (4 * CoordsY * CoordsY + CoordsX * CoordsX + CoordsZ * CoordsZ < 4
						&& ((world.getBlockState(new BlockPos(x + CoordsX, y + CoordsY, z + CoordsZ)))
								...
								|| (world.getBlockState(new BlockPos(x + CoordsX, y + CoordsY, z + CoordsZ)))
										.getBlock() == ElementureModBlocks.ACTIVENETHERUMSHINY.get())
					...
					world.setBlock(new BlockPos(x + CoordsX, y + CoordsY, z + CoordsZ), ElementureModBlocks.SUNTURFBLOCK.get().defaultBlockState(),
							3);
				}
				CoordsX = CoordsX + 1;
				if (CoordsX == 2) {
