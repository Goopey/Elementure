package net.mcreator.elementure.procedures;

import net.minecraft.world.level.block.Blocks;
import net.minecraft.world.level.LevelAccessor;
import net.minecraft.core.BlockPos;

import net.mcreator.elementure.ElementureMod;

public class DiverscrownlilypadGrassPatch3Procedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		double locX = 0;
		double locZ = 0;
		double rndRad = 0;
		double largeRndRad = 0;
		rndRad = 5 + Math.random() * 3;
		largeRndRad = rndRad * 2 + 1;
		locX = rndRad * (-1);
		locZ = rndRad * (-1);
		for (int index0 = 0; index0 < (int) (largeRndRad * largeRndRad); index0++) {
			if (locX * locX + locZ * locZ <= rndRad * rndRad) {
				if ((world.getBlockState(new BlockPos(x + locX, y - 1, z + locZ))).getBlock() == Blocks.GRASS_BLOCK) {
					world.setBlock(new BlockPos(x + locX, y, z + locZ), Blocks.GRASS_BLOCK.defaultBlockState(), 3);
					world.setBlock(new BlockPos(x + locX, y - 1, z + locZ), Blocks.DIRT.defaultBlockState(), 3);
				}
			}
			locX = locX + 1;
			if (locX > rndRad) {
				locX = rndRad * (-1);
				locZ = locZ + 1;
				if (locZ > rndRad) {
					locX = rndRad * (-1);
					locZ = rndRad * (-1);
				}
			}
		}
		ElementureMod.queueServerWork(2, () -> {
			DiverscrownBonemealGrassProcedure.execute(world, x, y, z);
		});
	}
}
