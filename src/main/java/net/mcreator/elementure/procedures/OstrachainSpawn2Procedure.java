...
public class OstrachainSpawn2Procedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		double yaw2 = 0;
		double yaw = 0;
		lengthChain = 3 + Math.round(Math.random() * 5);
		...
		while (num < lengthChain) {
			...
			if (world instanceof ServerLevel _level) {
				Entity entityToSpawn = new OstrachainLinkEntity(ElementureModEntities.OSTRACHAIN_LINK.get(), _level);
				entityToSpawn.moveTo((entity.getX() - num * 1.8 * yaw), (entity.getY()), (entity.getZ() - num * 1.8 * yaw2), 0, 0);
			...
			{
				final Vec3 _center = new Vec3((entity.getX() - num * 1.8 * yaw), (entity.getY()), (entity.getZ() - num * 1.8 * yaw2));
				...
				for (Entity entityiterator : _entfound) {
					if (entityiterator instanceof OstrachainLinkEntity) {
