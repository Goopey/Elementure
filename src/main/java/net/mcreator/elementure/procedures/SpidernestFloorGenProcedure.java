...
public class SpidernestFloorGenProcedure {
	/*
	...
	final static int DOOR_HEIGHT = 5;
	/*
	...
	public static void wallPatcher(LevelAccessor world, double x, double y, double z, boolean[] floorArray, int i, int xPos, int zPos) {
		...
		if (i < 82 && !floorArray[i + 9]) {
			SpidernestWallPatcherZProcedure.execute(world, x + (9 * xPos) + 4, y + DOOR_HEIGHT, z + (9 * zPos));
		}
		if (i > 18 && !floorArray[i - 9]) {
			SpidernestWallPatcherZProcedure.execute(world, x + (9 * xPos) - 4, y + DOOR_HEIGHT, z + (9 * zPos));
		}
		if (((i - 1) % 9 != 0) && !floorArray[i - 1]) {
			SpidernestWallPatcherXProcedure.execute(world, x + (9 * xPos), y + DOOR_HEIGHT, z + (9 * zPos) - 4);
		}
